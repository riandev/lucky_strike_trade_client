...
const Reports = () => {
  ...
  useEffect(() => {
    fetch("http://192.168.10.11:5006/reports")
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        setReport(data);
  ...
  useEffect(() => {
    fetch("http://192.168.10.11:5006/reportDates")
      .then((res) => res.json())
  ...
  function handlePrepare(pdate) {
    console.log(pdate);
    fetch("http://192.168.10.11:5006/prepareByDate?date=" + pdate)
      .then((res) => res.json())
                ...
                <td>
                  ...
                  >
                    <CSVLink
                      ...
                      filename={`LUCKY_STRIKE_TRADE_${date?.date}.csv`}
                      data={downloaded}
                    >
                      `Download_${date?.date}`
                    </CSVLink>
      ...
      <div style={{ display: report.length > 0 ? "block" : "none" }}>
        <button className="btn btn-danger mt-3">
          <CSVLink
            headers={headers}
            title="Export data to CSV"
            filename={"LUCKY_STRIKE_TRADE.csv"}
            data={report}
          >
            Download
          </CSVLink>
